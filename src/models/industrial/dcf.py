...
class IndustrialDCFModel(BaseDCF):
    def __init__(self, extracted_data: Dict[str, Any],
        ...
        self.discount_rate = extracted_data.get("valuation_parameters", {}).get("discount_rate", 0.075)
        self.fixed_growth = fixed_growth
        self.noi_multiplier = noi_multiplier

    def calculate(self) -> DCFResult:
        raise NotImplementedError("IndustrialDCFModel 尚未实现")

    def adjust(self, discount_rate=None, growth_rate=None, noi_multiplier=1.0) -> "IndustrialDCFModel":
        new = IndustrialDCFModel(self.data, self.detailed_data,
